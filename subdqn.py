...
class Args:
    ...
    beta: float = 1.0
    """the weight of the auxilary loss"""
...
if __name__ == "__main__":
    ...
    def l2(a):
        sums = jax.tree_util.tree_map(lambda x : jnp.sum(x**2), a)
        return jnp.sqrt(jax.tree_util.tree_reduce(lambda x, y: x + y, sums))
    ...
    def cosine_similarity(a, b):
        return dot_product(a,b) / (l2(a) * l2(b)) 
    ...
    def update(q_state, q_key, observations, actions, next_observations, rewards, dones):
        ...
        def mse_loss(params):
            paramss = optax.incremental_update(*params, alpha)
            q_pred = q_network.apply(paramss, observations)  # (batch_size, num_actions)
            q_pred = q_pred[jnp.arange(q_pred.shape[0]), actions.squeeze()]  # (batch_size,)
            cs = cosine_similarity(params[0], params[1])
            loss = ((q_pred - next_q_value) ** 2).mean() + args.beta * cs
            return loss, q_pred
